...
ENV GIT_REMOTE_URL http://gogs.apps.coe.igrtn.local/aqua/flow-storage
ENV GIT_CHECKOUT_BRANCH master
ENV FLOW_PROVIDER_GIT_FLOW_STORAGE_DIRECTORY $PROJECT_BASE_DIR/flow-storage
ENV FLOW_PROVIDER_GIT_REMOTE_TO_PUSH origin
ENV FLOW_PROVIDER_GIT_REMOTE_ACCESS_USER aqua
ENV FLOW_PROVIDER_GIT_REMOTE_ACCESS_PASSWORD aqua
ENV GIT_CONFIG_USER_NAME registry-bot
ENV GIT_CONFIG_USER_EMAIL registry-bot@local.local
...
RUN chmod -R 777 ${PROJECT_TEMPLATE_DIR}
RUN chmod -R 777 ${PROJECT_BASE_DIR}
